...
#x vey eksenlerine değişkenlerin isimleri verildi.
...
acoustic<-read_csv("C:/Users/yasem/OneDrive/Masaüstü/proje/acoustic.csv",
  ...
  #Date_M değişkeni  Yıl,ay,gün olarak tarih formatına dönüştürüldü.
  filter(Lon_M !=999.0) 
...
Sv_layer1 <- acoustic %>%
  ...
  #Variables_keep deki değişkenelr seçildi
...
<span style='color:maroon;font-weight:600'>*4)get_Interval_by_time();akustik verilerden Spatial_interval değişkeninin  bottom verisindeki akustik zamansal aralıklarla düşen noktalara Time_interval değerleri atayan bir işlevdir(buraya tekrar bak!!)*
...
<span style='color:maroon;font-weight:600'>*2)Grup bottom_spatial_interval_segments tarafından trackline_interval ve her aralık için ortalama derinliği özetler.*
...
Şimdi, yüksek ve düşük yoğunluklu balıkların rota hattı boyunca nerede olduğunu bulmak için Sv_mean'ı batimetriye (depth_plot) göre çizme zamanı. Y eksenlerimiz iki farklı ölçekte olduğundan, iki grafik oluşturacağız ve onları tek bir şekle yerleştirmek için patchwork paketini tekrar kullanacağız.
İhtiyacınız olan tüm veriler  Sv_and_depth de.
...
shelf break için ortalama derinlik=133 metre.
